...
使用 Speedcury ST2500 ATE 系统进行 IC 测试和特性分析的实践项目。重点关注混合信号 IC（ADC/OPA/PMIC）和测试自动化。

本仓库记录了我使用 Speedcury ST2500 ATE 系统进行混合信号 IC 测试的实践经验和数据分析。

项目列表

1. ADC – ADC0832CCN

重点：类 SPI 通信协议、混合信号同步、参考电压误差分析。

详情：文件夹待创建。

2. OPA – LM358P

重点：开环和闭环增益测量、BPMU 和 DGT 之间的协调、噪声基底特性分析。

3. PMIC – TPS73625

重点：线路调整率 (LNR)、电源抑制比 (PSRR)。

4. MCU – HK32F031

重点：GPIO 功能测试、基于 TMU 的频率捕获、自动化测试流程实现。

5. EEPROM – HK24C02

重点：I2C 协议实现、CBIT 控制的继电器切换（用于上拉电阻配置）。
